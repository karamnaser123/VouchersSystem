                            ...
                            <div class="col-md-4">
                                <div class="form-group mb-2 h-fill">
                                    <button type="submit" class="game-btn w-100">
                                        <i class="fas fa-search" aria-hidden="true"></i> @lang('Search')</button>
                                </div>
                            </div>
                        </div>
                    </form>
                        ...
                        @forelse($sellPostOffer as $k => $row)
                            <tr>
                                ...
                                <td data-label="@lang('Price')"><span
                                        class="font-weight-bold">{{ config('basic.currency_symbol') }}</span>{{ getAmount(@optional($row->sellPost)->price) }}
                                </td>
                                <td data-label="@lang('Offer Price')"><span
                                        class="font-weight-bold">{{ config('basic.currency_symbol') }}</span>{{ getAmount($row->amount) }}
                                </td>
                                                ...
                                                @if ($row->sellPost->payment_status != 1)
                                                    @if ($row->uuid)
                                                        <li><a class="dropdown-item offerAccept"
                                                                ...
                                                                <i class="text-white fa fa-comment-dots"></i>
                                                                @lang('Conversation')
                                                            </a>
                                                        </li>
                                                    @endif
