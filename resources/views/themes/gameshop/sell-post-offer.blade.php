                                            ...
                                            <div class="form-group mb-2">
                                                <select name="postId"
                                                    ...
                                                    @forelse($sellPostAll as $item)
                                                        <option @if (@request()->postId == $item->id) selected @endif
                                                            value="{{ $item->id }}">@lang($item->title)</option>
                                                    @empty
                                                    @endforelse
                                                </select>
                                            </div>
                                        ...
                                        <div class="col-md-6 col-lg-3">
                                            <div class="form-group mb-2">
                                                <input type="text" name="remark"
                                                    ...
                                                    class="form-control form-control-sm" placeholder="@lang('Type  user or message')">
                                            </div>
                                        </div>
                            ...
                            <div class="col-md-2">
                                <div class="btn-group btn-group-sm sortByBtn" role="group">
                                    <div class="btn-group" role="group">
                                        <button id="sortByActionBtn" type="button" class="btn text-white dropdown-toggle"
                        ...
                        <div class="offer-maker-list">
                            @forelse($sellPostOffer as $k => $row)
                                <div
                                    ...
                                    @if (optional($row->sellPost)->payment_lock == 1 &&
                                            optional($row->sellPost)->lock_for == $row->user_id &&
                                            \Carbon\Carbon::now() <
                                                Carbon\Carbon::parse(optional($row->sellPost)->lock_at)->addMinutes(config('basic.payment_expired'))) paid-making-payment @endif">
                                    <div class="flex-shrink-0 user">
                                        <a href="javascript:void(0)" title="{{ optional($row->user)->username }}"
                                    ...
                                    <div class="flex-grow-1 ms-3">
                                        ...
                                        <div class="d-flex justify-content-between align-items-center">
                                            <div class="d-flex justify-content-start ">
                                                ...
                                                <span class="ms-3 base-color">{{ optional($row->sellPost)->title }}</span>
                                            </div>
                                                        ...
                                                        @else
                                                            <li><a class="dropdown-item"
                                                                    href="{{ route('user.offerChat', $row->uuid) }}"><i
                                                                        class="text-success fa fa-comment"></i>
                                                                    @lang('Conversation')
                                                                </a>
                                                            </li>
                                                        @endif
                                                        ...
                                                        @if ($row->status == 0 || $row->status == 3)
                                                            <li><a class="dropdown-item offerReject"
                                                                    ...
                                                                    data-bs-target="#offerReject"><i
                                                                        class="text-danger fa fa-times"></i>
                                                                    @lang('Reject')
                                                                </a>
                                                            </li>
                                                        @endif
                                        ...
                                        <div class="d-flex ">
                                            ...
                                            @if (optional($row->sellPost)->payment_lock == 1 &&
                                                    optional($row->sellPost)->lock_for == $row->user_id &&
                                                    optional($row->sellPost)->payment_status == 0 &&
                                                    \Carbon\Carbon::now() <
                                                        Carbon\Carbon::parse(optional($row->sellPost)->lock_at)->addMinutes(config('basic.payment_expired')))
                                                <span class="badge bg-warning text-dark">@lang('Payment Processing')</span>
                                            @elseif(optional($row->sellPost)->payment_lock == 1 &&
                                                    optional($row->sellPost)->lock_for == $row->user_id &&
                                                    optional($row->sellPost)->payment_status == 1)
                                            ...
                                            <small class="ms-3"><i class="fa fa-clock"></i>
            ...
            <div class="modal-content ">
                ...
                <form action="{{ route('user.sellPostOfferAccept') }}" method="POST">
                    ...
                    <div class="modal-body-custom">
                        ...
                        <textarea name="description" rows="4" class="form-control custom mt-3" value="" required></textarea>
                        @error('description')
