                            ...
                            <form action="" method="get" id="searchFormSubmit">
                                <div class="input-group">
                                    <input type="text" name="search" class="form-control"
                                        placeholder="@lang('Search items')" value="{{ old('search', request()->search) }}"
                                        aria-label="Subscribe Newsletter" aria-describedby="basic-addon" />
                                ...
                                <div class="check-box">
                                    @forelse($categories as $category)
                                        <div class="form-check mb-3">
                                            <input class="form-check-input sortByCategory" name="sortByCategory"
                                                type="checkbox" value="{{ $category->id }}"
                                                @if (isset(request()->sortByCategory) && in_array($category->id, explode(',', request()->sortByCategory))) checked @endif
                            ...
                            <div class="col-md-6 d-flex mt-4 mt-md-0 justify-content-md-end align-items-center">
                                ...
                                <form action="" method="get" id="sortBy">
                                    <select name="sortBy" class="form-control form-select"
                                        ...
                                        <option selected value="latest" @if (request()->sortBy == 'latest') selected @endif>
                                            @lang('Latest')</option>
                                        <option value="low_to_high" @if (request()->sortBy == 'low_to_high') selected @endif>
                                            @lang('Price low to high')
                                        </option>
                                        <option value="high_to_low" @if (request()->sortBy == 'high_to_low') selected @endif>
                                    ...
                                    <div class="game-box d-md-flex">
                                        @if ($item->image)
                                            <div class="img-box image-slider owl-carousel">
                                                @for ($i = 0; $i < count($item->image); $i++)
                                                    <img src="{{ getFile(config('location.sellingPost.path') . @$item->image[$i]) }}"
                                                        class="img-fluid" alt="..." />
                                                @endfor
                                            </div>
                                        @endif
                                        <div>
                                            <a href="{{ route('sellPost.details', [@slug($item->title), $item->id]) }}">
                                                <h5 class="name">{{ \Illuminate\Support\Str::limit($item->title, 25) }}
                                                </h5>
                                                <div class="d-flex justify-content-between">
                                                    <span class="game-level">@lang('Price'):
                                                        <span>{{ getAmount($item->price) }}
                                                            {{ config('basic.currency') }}</span></span>
                                        ...
                                        @if (Auth::check() && $item->user_id != Auth::user()->id)
                                            @if ($item->payment_lock == 0)
                                                <button class="game-btn-sm makeOffer" data-resource="{{ $item->id }}"
                                                    ...
                                                    @lang('make offer')
                                                    <img src="{{ asset($themeTrue) . '/images/icon/arrow-white.png' }}"
                                                        alt="..." />
                                                </button>
                                            @endif
