...
class FrontendController extends Controller
{
    ...
    public function shop(Request $request)
    {
        if (config('basic.top_up')) {
            $topUp = Category::with(['details'])->withCount('totalSold')->whereStatus(1)
                ->whereNotIn('appStoreLink', ['1', '2', '3', '4', '5'])
                ->get();
        } else {
            $topUp = collect();
        }
        if (config('basic.top_up')) {
            $topUp2 = Category::with(['details'])->withCount('totalSold')->whereStatus(1)
                ->where('appStoreLink', '1')
                ->get();
        } else {
            $topUp2 = collect();
        }
        if (config('basic.top_up')) {
            $topUp3 = Category::with(['details'])->withCount('totalSold')->whereStatus(1)
                ->where('appStoreLink', '2')
                ->get();
        } else {
            $topUp3 = collect();
        }
        if (config('basic.top_up')) {
            $topUp4 = Category::with(['details'])->withCount('totalSold')->whereStatus(1)
                ->where('appStoreLink', '3')
                ->get();
        } else {
            $topUp4 = collect();
        }
